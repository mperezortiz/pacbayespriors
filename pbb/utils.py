...
def runexp(experiment_args, prior_args, posterior_args):
    ...
    if prior_net_params["objective"] in ['erm', 'mixup'] and objective in ['erm', 'mixup']:
        if vision_dataset:
            ...
            X_train, X_test, y_train, y_test = data.holdout_data(
            ...
            if perc_train < 1.0:
                X_train, _, y_train, _ = data.holdout_data(
                    X_train, y_train, random_state=RANDOM_STATE, test_size=1-perc_train)

            if perc_val > 0.0:
                X_train, X_val, y_train, y_val = data.holdout_data(
                    X_train, y_train, random_state=RANDOM_STATE, test_size=perc_val)
            ...
            prior_net_params["loader"] = data.load_batches(
                X_post, y_post, loader_kargs)
            test_loader = data.load_batches(X_test, y_test, loader_kargs)

            train_size = X_post.shape[0]
            num_classes = np.unique(y_post).shape[0]

        prior_net = initialise_prior(model, name_data, layers, prior_net_params['dropout_prob'], device, prior_net_params["objective"], rho_prior, prior_net_params[
                                     'weight_distribution'], init_priors_prior=prior_net_params["initialize_prior"], features=n_features, classes=num_classes, neurons=neurons)
        ...
        prior_net = train_prior_net(
            prior_net, prior_net_params, experiment_settings, val_loader=val_loader)
        loss_prior_net, error_prior_net = testNNet(
            prior_net, test_loader, device=device)
        ...
        if vision_dataset:
            ...
            if prior_type == 'learnt':
                ...
                X_eval, X_prior, y_eval, y_prior = data.holdout_data(
                    X_train, y_train, random_state=RANDOM_STATE, test_size=perc_prior)

                if perc_val > 0.0:
                    # we divide data for the prior again to set aside some validation data
                    X_prior, X_val, y_prior, y_val = data.holdout_data(
                        X_prior, y_prior, random_state=RANDOM_STATE, test_size=perc_val)
                    val_loader = data.load_batches(X_val, y_val, loader_kargs)
                else:
                    val_loader = None

                if oversampling:
                    _, _, X_prior, y_prior = data.oversample(
                        X_prior, y_prior, k_oversampling=k_oversampling, random_state=RANDOM_STATE, perc_oversampling=perc_oversampling)
                    # the posterior is learnt with all the data + synthetic data from all the data
                    _, _, X_post, y_post = data.oversample(
                        X_train, y_train, k_oversampling=k_oversampling, random_state=RANDOM_STATE, perc_oversampling=perc_oversampling)
            ...
            bound_loader_1batch = data.load_batches(
                X_eval, y_eval, loader_kargs, batch_size=-1)
            train_size = X_post.shape[0]
            num_classes = np.unique(y_post).shape[0]

        if prior_type == 'learnt':
            prior_net_params['n_prior'] = len(
                prior_net_params["loader"])*batch_size
        else:
        ...
        if prior_net_params['objective'] == 'flamb':
            prior_net_params['lambda_var'] = Lambda_var(
                initial_lamb, train_size).to(device)
            prior_net_params['optimizer_lambda'] = optim.SGD(
        ...
        if prior_net_params["objective"] in ['bbb', 'fquad', 'fclassic']:
            to_log_prior = validate_posterior_net(
                prior_net, prior_net_params, experiment_settings)
        ...
        posterior_net, toolarge = initialise_posterior(
            model, name_data, layers, rho_prior, prior_net_params['weight_distribution'], prior_net, device, features=n_features, classes=num_classes, neurons=neurons)
        _, stch_err_mean_prior, stch_err_std_prior = testStochastic(
            posterior_net, test_loader, posterior_bound, device=device)
        ...
        posterior_net = train_posterior_net(
            posterior_net, posterior_net_params, experiment_settings)

        to_log = validate_posterior_net(
            posterior_net, posterior_net_params, experiment_settings)
    ...
    if model == 'cnn':
        ...
        if name_data == 'mnist':
            if prob_prior:
                ...
                if layers == 4:
                    prior_net = NNet4l(
                        dropout_prob=dropout_prob, device=device).to(device)
                else:
                    prior_net = NNet3l(dropout_prob=dropout_prob, device=device,
                                       features=784, classes=10, neurons=neurons).to(device)
        else:
            if prob_prior:
                if layers == 4:
                ...
                if layers == 4:
                    prior_net = NNet4l(dropout_prob=dropout_prob, device=device,
                                       features=features, classes=classes, neurons=neurons).to(device)
                else:
                    prior_net = NNet3l(dropout_prob=dropout_prob, device=device,
                                       features=features, classes=classes, neurons=neurons).to(device)
...
def initialise_posterior(model, name_data, layers, rho_prior, prior_dist, prior_net, device, features=28*28, classes=10, neurons=100):
    ...
    if model == 'cnn':
        ...
        if name_data == 'cifar10':
            raise RuntimeError(
                f'Cifar10 not supported with given architecture {model}')
        elif name_data == 'mnist':
            if layers == 4:
...
def train_prior_net(prior_net, prior_net_params, experiment_settings, val_loader=None):
    ...
    if objective_prior == 'bbb' or objective_prior == 'fquad':
        ...
        for epoch in trange(epochs):
            trainPNNet(prior_net, prior_optimizer, bound_prior, epoch, loader, lambda_var,
                       optimizer_lambda, True, log_wandb=wandb_args['log_wandb'], wandb_is_prior=True)
            if val_loader:
                val_loss_prior_net, _, _ = testStochastic(
                    prior_net, val_loader, bound_prior, device=device, samples=1)
        ...
        for epoch in trange(epochs):
            ...
            if val_loader:
                val_loss_prior_net, _ = testNNet(
                    prior_net, val_loader, device=device)
...
def train_posterior_net(posterior_net, posterior_net_params, experiment_settings):
    ...
    posterior_optimizer = optim.SGD(posterior_net.parameters(
    ), lr=learning_rate_posterior, momentum=momentum_posterior)
    ...
    for epoch in trange(epochs):
        trainPNNet(posterior_net, posterior_optimizer, bound, epoch, loader,
                   lambda_var, optimizer_lambda, verbose, wandb_args['log_wandb'])
        if verbose_test and ((epoch+1) % freq_test == 0):
            #import ipdb; ipdb.set_trace()
            to_log = validate_posterior_net(
                posterior_net, posterior_net_params, experiment_settings, compute_all_metrics=False)
...
def validate_posterior_net(posterior_net, posterior_net_params, experiment_settings, compute_all_metrics=True):
    ...
    if compute_all_metrics:
        stch_loss, stch_err_mean, stch_err_std = testStochastic(
            posterior_net, test_loader, bound, device=device, samples=10)
        post_loss, post_err = testPosteriorMean(
            posterior_net, test_loader, bound, device=device)
        ens_loss, ens_err = testEnsemble(
            posterior_net, test_loader, bound, device=device, samples=samples_ensemble)
    else:
